/* JPT V106 CUSTOMER MENU CONNECTION BRIDGE — CART HOTFIX
   Safe replacement for customer-menu-runtime-fix-v106.js
   Reads public menu data only. Does not write/delete/import menu or order data.
   Fix: ADD buttons use the original page-level `change(id, delta)` handler.
*/
(function(){
'use strict';
...
function ensureClient(){
 ...
 if(window.supabase&&window.JPT_SUPABASE_URL&&window.JPT_SUPABASE_PUBLISHABLE_KEY){client=window.supabase.createClient(window.JPT_SUPABASE_URL,window.JPT_SUPABASE_PUBLISHABLE_KEY);window.sb=client;return client}
 return null
}
function renderOutlets(){
 ...
 Array.prototype.forEach.call(bar.querySelectorAll('[data-jpt-bridge-outlet]'),function(b){b.onclick=function(){var id=b.getAttribute('data-jpt-bridge-outlet');window.outletId=id;try{history.pushState({outlet:id},'',location.pathname+'?outlet='+encodeURIComponent(id))}catch(e){};renderOutlets();load(true)}})
}
function renderCats(rows){
 ...
 Array.prototype.forEach.call(chips.querySelectorAll('[data-jpt-bridge-cat]'),function(b){b.onclick=function(){Array.prototype.forEach.call(chips.querySelectorAll('.chip'),function(x){x.classList.remove('on')});b.classList.add('on');var c=b.getAttribute('data-jpt-bridge-cat');renderMenu(c==='All'?lastRows:lastRows.filter(function(x){return String(x.category||'')===c}),lastMap)}})
}
function renderMenu(rows,map){
 ...
 cats.forEach(function(cat){
  ...
  html+=arr.map(function(x){
   ...
   var addHandler='change('+JSON.stringify(String(x.id))+',1)';
   return '<div class="item"><div class="pic">'+(src?'<img src="'+esc(src)+'" alt="'+esc(x.name||'')+'" loading="lazy">':'<span>IMAGE<br>AVAILABLE FROM ADMIN</span>')+'</div><div class="info"><div class="dish">'+esc(x.name||'')+'</div><div class="desc">'+esc(x.description||'')+'</div><div class="price">'+money(x.price)+'</div><button class="add" type="button" data-jpt-add="'+esc(x.id)+'" onclick="'+esc(addHandler)+'">ADD +</button></div></div>'
  }).join('')
 });
 menu.innerHTML=html;
 Array.prototype.forEach.call(menu.querySelectorAll('[data-jpt-add]'),function(b){b.addEventListener('click',function(){var id=b.getAttribute('data-jpt-add');if(typeof window.jptChange==='function'){try{window.jptChange(id,1)}catch(e){}}})})
}
async function load(force){
 ...
 try{
  ...
  document.documentElement.setAttribute('data-jpt-v106-cart-bridge','inline-change-v2');
