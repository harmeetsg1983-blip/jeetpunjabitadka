/* JPT V106 CUSTOMER MENU CONNECTION BRIDGE — CART HOTFIX V2
   Non-destructive. Reads menu data only.
   Critical fix: explicitly exposes the existing page-level `change()` cart
   function as window.jptChange before the bridge renders ADD buttons.
*/
(function(){
'use strict';
...
function exposeCart(){
  try{
    if(typeof change==='function'){
      window.jptChange=change;
      return true
    }
  }catch(e){}
  return typeof window.jptChange==='function'
}
...
function ensureClient(){
  ...
  if(window.sb&&typeof window.sb.from==='function'){
    client=window.sb;return client
  }
...
function renderMenu(rows,map){
  ...
  Array.prototype.forEach.call(
    ...
    function(b){
      b.onclick=function(){
        var id=b.getAttribute('data-jpt-add');
        if(exposeCart()){
          try{window.jptChange(id,1)}catch(e){
            console.warn('JPT cart add failed',e)
          }
        }else{
          console.warn('JPT cart handler not exposed yet')
        }
      }
    }
  )
...
async function load(force){
  ...
  try{
    ...
    document.documentElement.setAttribute('data-jpt-v106-cart-bridge','explicit-change-v2');
