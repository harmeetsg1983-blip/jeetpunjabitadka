...
(function(){
  ...
  async function saveOrder(id, patch){
    ...
    if(r.error){
      msg('Order update failed: '+r.error.message);
      return false;
    }
  ...
  window.loadOrders = async function(){
    const r=await outletFilter(
      sb.from('orders')
        .select('*')
    ).order('created_at',{ascending:false}).limit(50);

    if(r.error){
      msg('Orders: '+r.error.message);
      return;
    }
    ...
    $('ordersList').innerHTML=data.length ? data.map(o=>{
      ...
      const target=
        allowed.includes(Number(o.target_minutes))
          ? Number(o.target_minutes)
          : 30;

      const acceptedAt=o.accepted_at || '';

      const deadline=o.deadline_at || (
        acceptedAt
          ? new Date(
              new Date(acceptedAt).getTime()+target*60000
            ).toISOString()
          : ''
      );
      ...
      const action = o.status==='new'
        ? `<div class="partnerActions">
             ...
             <div class="small" style="margin-bottom:7px">
               Select preparation time, then accept:
             </div>

             <div class="timerChoices">
               ${allowed.map(m=>`
                 <button
                   type="button"
                   class="timerChoice ${m===target?'active':''}"
                   data-po-timer="${esc2(o.id)}"
                   data-min="${m}">
                   ${m} min
                 </button>
               `).join('')}
        ...
        : `<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px">
             ...
             <button
               type="button"
               class="btn ${o.status==='preparing'?'primary':'dark'}"
               data-po-prep="${esc2(o.id)}">
               👨‍🍳 ${o.status==='preparing'?'PREPARING':'START PREPARING'}
             </button>

             <button
               type="button"
               class="btn online"
               data-po-ready="${esc2(o.id)}">
               🟢 READY
             </button>
      ...
      return `<div class="order ${o.status==='new'?'new':''}">
        ...
        <div class="order-top">
          ...
          <div>
            <b>
              ${esc2(o.order_no||('#'+String(o.id).slice(0,8)))}
            </b>
        ...
        <div style="margin:8px 0">
          ...
          <b>
            ${esc2(o.customer_name||'Customer')}
          </b>

          ${
            o.customer_phone
              ? ' · '+esc2(o.customer_phone)
              : o.phone
                ? ' · '+esc2(o.phone)
                : ''
          }
          ...
          ${esc2(o.customer_address||o.address||'')}
            ...
            : `
                <div class="item">
                  <div style="color:#ffcf6b">
                    ⚠️ Item details unavailable in order record.
                  </div>
                </div>
              `
        }
        ...
        <div class="row" style="margin-top:9px">
          ...
          <span>
            Subtotal ${money2(o.subtotal)}
            · Discount ${money2(o.discount)}
          </span>
        ...
        <div class="timerBox">
          ...
          <div class="row">
            ...
            <span>
              <b>⏱️ Preparation Timer</b>
            </span>

            <span
              class="countdown ${timerClass}"
              data-po-deadline="${esc2(deadline)}">
          ...
          <div class="small">
            ...
            ${
              o.status==='new'
                ? 'Choose 30/40/50/60 minutes before accepting.'
                : `Target: ${target} min${
                    acceptedAt
                      ? ' · accepted '+new Date(acceptedAt).toLocaleTimeString()
                      : ''
                  }`
            }
    ...
    document.querySelectorAll('[data-po-timer]').forEach(b=>{
      ...
      b.onclick=async()=>{
        ...
        const minutes=
          Math.min(
            ...
            Math.max(
              30,
              Number(b.dataset.min)||30
            )
          );

        document.querySelectorAll(
          `[data-po-timer="${CSS.escape(b.dataset.poTimer)}"]`
        ).forEach(x=>x.classList.remove('active'));

        b.classList.add('active');

        b.closest('.order')
          ?.querySelectorAll('[data-po-accept]')
          .forEach(x=>{
            x.dataset.selectedMinutes=String(minutes);
          });

        msg(
          `Preparation target selected: ${minutes} minutes.`
        );
      };
    ...
    document.querySelectorAll('[data-po-accept]').forEach(b=>{
      ...
      b.onclick=async()=>{
        ...
        const minutes=
          Number(
            b.dataset.selectedMinutes ||
            b.closest('.order')
              ?.querySelector('.timerChoice.active')
              ?.dataset.min ||
            30
          );
        ...
        const saved=
          await saveOrder(
            b.dataset.poAccept,
            {
              ...
              eta_minutes:minutes+20
            }
          );

        /* Stop alarm ONLY after successful ACCEPT */
        ...
        msg(
          `✅ Order accepted — ${minutes} min preparation + 20 min delivery buffer.`
        );
    ...
    document.querySelectorAll('[data-po-reject]').forEach(b=>{
      ...
      b.onclick=async()=>{
        ...
        const saved=
          await saveOrder(
            ...
            {
              status:'cancelled',
              rejection_reason:'Rejected by restaurant'
            }
          );

        /* Stop alarm ONLY after successful REJECT */
    ...
    document.querySelectorAll('[data-po-prep]').forEach(b=>{
    ...
    document.querySelectorAll('[data-po-ready]').forEach(b=>{
  ...
  function boot(){
    ...
    style.textContent=
      '.partnerActions{margin-top:10px;padding:10px;border:1px solid #4b3b1e;border-radius:12px;background:#17130d}.partnerActions .timerChoices{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}.partnerActions .timerChoice{min-height:42px}';
    ...
    try{
      ...
      const r=
        await sb.from('orders')
          .select(
            'id,created_at,status,order_no,outlet_id'
          )
          .eq(
            'outlet_id',
            currentOutlet
...
/* JPT V106 LOUD RESTAURANT ALERT — FINAL
   Continuous restaurant-style alert.
   Alarm stops only after ACCEPT or REJECT succeeds.
*/
