/* JPT Restaurant Partner Onboarding UI V2
   V1 UI plus secure Central Owner approval automation.
   Existing dashboard/customer/order/menu logic remains untouched.
*/
(function(){
  'use strict';
  if(window.__JPT_PARTNER_ONBOARDING_UI_V2__) return;
  window.__JPT_PARTNER_ONBOARDING_UI_V2__=true;
  ...
  function addStyle(){
    if($('jpt-onboarding-style-v2')) return;
    const s=document.createElement('style'); s.id='jpt-onboarding-style-v2';
    s.textContent=`
  ...
  async function approveRestaurant(id){
    if(!confirm('Approve this restaurant and create its Outlet ID + Partner access?')) return;
    const b=document.querySelector('[data-ob-approve="'+CSS.escape(id)+'"]');
    if(b){b.disabled=true;b.textContent='APPROVING…'}
    const r=await sb.functions.invoke('partner-approve-restaurant',{body:{application_id:id}});
    if(r.error){
      toast?.('Approval failed: '+(r.error.message||'Function error'));
      if(b){b.disabled=false;b.textContent='APPROVE'}
      return;
    }
    const data=r.data||{};
    if(data.error){
      toast?.('Approval failed: '+data.error);
      if(b){b.disabled=false;b.textContent='APPROVE'}
      return;
    }
    toast?.('Approved • Outlet ID '+(data.outlet_id||'created'));
    await load();
  }
  ...
  async function load(){
    ...
    $('jptObList').innerHTML=apps.map(a=>`
      <div class="jpt-ob-row">
        <div class="grow">
          ...
          ${a.outlet_id?'<div class="muted">Outlet ID: <b>'+esc(a.outlet_id)+'</b></div>':''}
        </div>
        <div>
    ...
    document.querySelectorAll('[data-ob-approve]').forEach(b=>b.onclick=()=>approveRestaurant(b.dataset.obApprove));
